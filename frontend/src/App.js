...
import { Upload, TrendingUp, Target, Search, Zap, Users, Trophy, Calendar, Brain } from 'lucide-react';
...
export default function RocketLeagueDashboard() {
  ...
  const fetchTrends = useCallback(async (playerId) => {
    try {
                  ...
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                    <div>
                      ...
                      <div style={{color: '#9ca3af', fontSize: '14px'}}>
                        {match.match_date ? new Date(match.match_date).toLocaleDateString() : 'Unknown Date'} • {match.game_mode || 'Standard'}
                      </div>
                      {/* Players per team */}
                      <div style={{display: 'flex', gap: '24px', marginTop: '12px', flexWrap: 'wrap'}}>
                        <div>
                          <div style={{color: '#93c5fd', fontWeight: 700, marginBottom: '8px'}}>Blue Team</div>
                          <div style={{display: 'flex', gap: '8px', flexWrap: 'wrap'}}>
                            {(match.players || []).filter(p => p.team === 'blue').map((p, idx) => (
                              <span key={`b-${idx}`} style={{...styles.badge, background: 'rgba(59, 130, 246, 0.12)', color: '#93c5fd'}}>
                                {p.name}
                              </span>
                            ))}
                          </div>
                        ...
                        <div>
                          <div style={{color: '#fdba74', fontWeight: 700, marginBottom: '8px'}}>Orange Team</div>
                          <div style={{display: 'flex', gap: '8px', flexWrap: 'wrap'}}>
                            {(match.players || []).filter(p => p.team === 'orange').map((p, idx) => (
                              <span key={`o-${idx}`} style={{...styles.badge, background: 'rgba(249, 115, 22, 0.12)', color: '#fdba74'}}>
                                {p.name}
                              </span>
                            ))}
                          </div>
